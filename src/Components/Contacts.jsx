      ...
      <div className="contacts main_container p-12 bg-white border-4  border-black ">
        ...
        <div className="contact Child active">
          ...
          <div className="filter-input relative">
            <input type="text" placeholder="Filter by Name, Desc, Phone number ...." onChange={(event) => setSearchTerm(event.target.value)} className='w-full  border-b-4 border-black mb-4 pt-[2px] pl-6 focus:border-[#ffa580] outline-none'/>
        ...
        <div className="Child form hidden">
        	...
        	<div className="head">
         	...
         	<form className=' flex flex-col space-y-4' onSubmit={(e) => { e.preventDefault(); document.querySelectorAll("input:not([type='submit'])").forEach((e) => (e.value = ""));}}>
         			<input className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none" placeholder="Name" required type="text" id="name" name="name" onChange={(e) => { setName(e.target.value); }} />
         			<input className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none" placeholder="Phone number" required type="number" id="phone" name="phone" onChange={(e) => { setPhone(e.target.value); }}/>
         			<input className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none" placeholder="Email" required type="email" id="email" name="email" onChange={(e) => { setEmail(e.target.value); }} />
         			<input className="links bg-[#95a4ff]  flex items-center px-20 border-2 border-black hover:bg-black hover:text-white h-8 cursor-pointer" type="submit" value="Create" onClick={() => dispatch(addContact({ name: name, phone: phone, email: email }))} />
         	</form>
    	</div>
