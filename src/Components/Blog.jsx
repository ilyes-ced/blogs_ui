    ...
    <>
      <div className="blogs main_container p-12 bg-white border-4  border-black ">
                    ...
                    <td className="w-1/5  border border-black">
                      {post.commit.length === 0 ? (
                        <div className="commit flex flex-row">
                          <input type="text" onChange={(e) => setCommit(e.target.value)} className='' placeholder="Commit" />
                          <button className="bg-[#ffa580]  border border-black transition-all duration-200 ease-in-out hover:bg-black hover:text-white " onClick={(e) => { dispatch(addCommit([post.id, commit])); }} > commit </button>
                        </div>
                      ) : (
                        post.commit
                      )}
        ...
        <h2 className="text-2xl font-bold mb-4">Create Blogs</h2>
          ...
          >
            <input placeholder="Subject" className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none pl-2"
              required
              type="text"
              id="subject"
              name="subject"
            ...
            />
            <input placeholder="Description" className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none pl-2"
              required
              type="text"
              id="description"
              name="description"
            ...
            />
            <input  className=" text-lg border-b-4 border-black pt-2 focus:border-[#ffa580] outline-none pl-2"
              type="date"
              id="date"
              name="date"
              required
            ...
            />
            <input
              type="submit" className="links bg-[#95a4ff]  flex items-center px-20 border-2 border-black hover:bg-black hover:text-white h-8 cursor-pointer"
              value="Add new Post"
              onClick={() =>
                dispatch(
                  addPost({
